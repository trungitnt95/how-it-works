...
(function() {
    ...
    const state = {
        currentLevel: localStorage.getItem('managementLevel') || null,
        currentTab: 'simple',
        isTourActive: false,
        tourStepIndex: 0,
    ...
    function selectLevel(level) {
        ...
        localStorage.setItem('managementLevel', level);
        hideOverlay();
        updateLevelUI();
        filterNodesByLevel();
    }
    ...
    function initChecklistModal() {
        ...
        elements.resetChecklist.addEventListener('click', () => {
            localStorage.removeItem('managementChecklist');
            renderChecklist();
        });
    ...
    function renderChecklist() {
        const saved = JSON.parse(localStorage.getItem('managementChecklist') || '{}');
        elements.checklistItems.innerHTML = managementChecklist.map((item, i) => `
        ...
        elements.checklistItems.querySelectorAll('input').forEach(cb => {
            cb.addEventListener('change', () => {
                ...
                localStorage.setItem('managementChecklist', JSON.stringify(checks));
            });
        });
    }
