// Philosophy & Applications - Main App
(function() {
    'use strict';

    const state = {
        currentLevel: localStorage.getItem('philosophyLevel') || null,
        currentTab: 'simple',
        isTourActive: false,
        tourStepIndex: 0,
    ...
    const allComponents = {
        ...philosophyDialecticalData,
        ...philosophyHistoricalData,
        ...philosophyPoliticalEconomyData,
        ...philosophySocialismData,
        ...philosophyAppliedData
    };
    ...
    function selectLevel(level) {
        ...
        localStorage.setItem('philosophyLevel', level);
        hideOverlay();
        updateLevelUI();
        filterNodesByLevel();
    }
    ...
    function updateLevelUI() {
        const names = { beginner: '📗 Ôn Cơ Bản', intermediate: '📘 Học Để Hiểu', advanced: '📕 Ứng Dụng Thực Tế' };
    ...
    function initChecklistModal() {
        ...
        elements.resetChecklist.addEventListener('click', () => {
            localStorage.removeItem('philosophyChecklist');
            renderChecklist();
        });
    ...
    function renderChecklist() {
        const saved = JSON.parse(localStorage.getItem('philosophyChecklist') || '{}');
        elements.checklistItems.innerHTML = philosophyChecklist.map((item, i) => `
        ...
        elements.checklistItems.querySelectorAll('input').forEach(cb => {
            cb.addEventListener('change', () => {
                ...
                localStorage.setItem('philosophyChecklist', JSON.stringify(checks));
            });
        });
    }
