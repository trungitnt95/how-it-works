// Practical Math - Main App
(function() {
    'use strict';

    const state = {
        currentLevel: localStorage.getItem('mathLevel') || null,
        currentTab: 'simple',
        isTourActive: false,
        tourStepIndex: 0,
    ...
    function initChecklistModal() {
        ...
        elements.resetChecklist.addEventListener('click', () => {
            localStorage.removeItem('mathChecklist');
            renderChecklist();
        });
    ...
    function renderChecklist() {
        const saved = JSON.parse(localStorage.getItem('mathChecklist') || '{}');
        elements.checklistItems.innerHTML = mathChecklist.map((item, i) => `
        ...
        elements.checklistItems.querySelectorAll('input').forEach(cb => {
            cb.addEventListener('change', () => {
                ...
                localStorage.setItem('mathChecklist', JSON.stringify(checks));
            });
        });
    }
