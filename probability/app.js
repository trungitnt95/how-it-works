// Probability & Statistics - Main App
(function() {
    'use strict';

    const state = {
        currentLevel: localStorage.getItem('probabilityLevel') || null,
        currentTab: 'simple',
        isTourActive: false,
        tourStepIndex: 0,
    ...
    // Merge all data
    const allComponents = {
        ...probabilityBasicsData,
        ...probabilityConceptsData,
        ...distributionsData,
        ...probabilityFallaciesData,
        ...probabilityApplicationsData
    };
    ...
    function selectLevel(level) {
        ...
        localStorage.setItem('probabilityLevel', level);
        hideOverlay();
        updateLevelUI();
        filterNodesByLevel();
    }
    ...
    function initChecklistModal() {
        ...
        elements.resetChecklist.addEventListener('click', () => {
            localStorage.removeItem('probabilityChecklist');
            renderChecklist();
        });
    ...
    function renderChecklist() {
        const saved = JSON.parse(localStorage.getItem('probabilityChecklist') || '{}');
        elements.checklistItems.innerHTML = probabilityChecklist.map((item, i) => `
        ...
        elements.checklistItems.querySelectorAll('input').forEach(cb => {
            cb.addEventListener('change', () => {
                ...
                localStorage.setItem('probabilityChecklist', JSON.stringify(checks));
            });
        });
    }
