...
(function() {
    ...
    const state = {
        currentLevel: localStorage.getItem('psychologyLevel') || null,
        currentTab: 'simple',
        isTourActive: false,
        tourStepIndex: 0,
    ...
    function selectLevel(level) {
        ...
        localStorage.setItem('psychologyLevel', level);
        hideOverlay();
        updateLevelUI();
        filterNodesByLevel();
    }
    ...
    function initChecklistModal() {
        ...
        elements.resetChecklist.addEventListener('click', () => {
            localStorage.removeItem('psychologyChecklist');
            renderChecklist();
        });
    ...
    function renderChecklist() {
        const saved = JSON.parse(localStorage.getItem('psychologyChecklist') || '{}');
        elements.checklistItems.innerHTML = psychologyChecklist.map((item, i) => `
        ...
        elements.checklistItems.querySelectorAll('input').forEach(cb => {
            cb.addEventListener('change', () => {
                ...
                localStorage.setItem('psychologyChecklist', JSON.stringify(checks));
            });
        });
    }
